...
async function handleWatchedEpisode(
  ...
  if (animeName) {
    ...
    if (anilistEntry) {
      await updateAnilist(anilistEntry.id, currentEpisode, preAddedOnly, adultPrivate, token);
    }
  } else if (anilistId) {
    await updateAnilist(anilistId, currentEpisode, preAddedOnly, adultPrivate, token);
  }
}
...
async function updateAnilist(id, currentEpisode, preAddedOnly, adultPrivate, token) {
  const anilistApi = new Anilist(token);
  ...
  const makePrivate = adultPrivate && currAnime.isAdult ? true : false;
